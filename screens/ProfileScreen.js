    ...
    return (
        <KeyboardAwareScrollView style={styles.container}>
            <SafeAreaView>
                        ...
                        {  /* added the part key={index} to Fix the warning "Each child should have a unique key prop"*/
                            pictures.map((e, index) =>
                                <TouchableOpacity styles={{ marginLeft: 50 }} onPress={() => { changeImage(index + 1); }} key={index}>
                                    <Image source={{ uri: `data:image/png;base64,${e}` }} style={styles.image} />
                                </TouchableOpacity>
                            )
                        }
                ...
                <TextInput
                    value={name}
                    onChangeText={value => { dispatch(setName(value)); console.log(name) }}
                    placeholder={'Name'}
                ...
                <View style={styles.horizontal}>
                    ...
                    <View>
                        ...
                        <TextInput
                            value={location}
                            onChangeText={(location) => { dispatch(setLocation(location)) }}
                ...
                <TextInput
                    value={bio}
                    onChangeText={(bio) => { dispatch(setBio(bio)) }}
                    placeholder={'Bio'}
